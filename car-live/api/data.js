...
// This dashboard is explicitly an all-campaign dashboard. Prefer the dedicated
// *_ALL_URL queries. The plain URLs are retained only as a fallback when an
// all-campaign query has not been configured for a platform.
function sourceConfig(allUrl,standardUrl,allName,standardName){
  if(allUrl) return {url:allUrl,scope:'all-campaigns',sourceEnv:allName};
  if(standardUrl) return {url:standardUrl,scope:'fallback-standard',sourceEnv:standardName};
...
function supermetricsDiagnostics(payload,range){
  ...
  return {
    ...
    sourceRange:{start_date:query.start_date||null,end_date:query.end_date||null},
...
export default async function handler(req,res){
  ...
  const liveSources={
    Meta:sourceConfig(process.env.SM_META_ALL_URL,process.env.SM_META_URL,'SM_META_ALL_URL','SM_META_URL'),
    Google:sourceConfig(process.env.SM_GOOGLE_ALL_URL,process.env.SM_GOOGLE_URL,'SM_GOOGLE_ALL_URL','SM_GOOGLE_URL'),
    Snapchat:sourceConfig(process.env.SM_SNAPCHAT_ALL_URL,process.env.SM_SNAPCHAT_URL,'SM_SNAPCHAT_ALL_URL','SM_SNAPCHAT_URL'),
    TikTok:sourceConfig(process.env.SM_TIKTOK_ALL_URL,process.env.SM_TIKTOK_URL,'SM_TIKTOK_ALL_URL','SM_TIKTOK_URL'),
    LinkedIn:sourceConfig(process.env.SM_LINKEDIN_ALL_URL,process.env.SM_LINKEDIN_URL,'SM_LINKEDIN_ALL_URL','SM_LINKEDIN_URL')
  };
  ...
  await Promise.all(Object.entries(liveSources).map(async([name,cfg])=>{
    ...
    try{
      ...
      out[name]={ok:true,data,refreshMode:'30min',queryScope:cfg.scope,sourceEnv:cfg.sourceEnv,diagnostics};
    }catch(e){
  ...
  try{
    ...
    const r=await fetch(`${proto}://${host}/api/daily${qs}`,{cache:'no-store'});
